...
const (
	data = `"281470816482304","281470816482559","CA","Canada","Quebec","Montreal","45.508840","-73.587810","H1A 0A1","-04:00"
...
func TestSearch(t *testing.T) {
	...
	ip, _ := Search("8.8.8.8", []string{dataFile.Name()}...)
	...
	ip, _ = Search("2001:4860:4860:0:0:0:0:8888", []string{dataFile.Name()}...)
...
func Test_openCSV(t *testing.T) {
	...
	file, err := openCSV("../cmd/app/" + dataFile.Name())
	assert.NotNil(t, err)
	assert.Nil(t, file)

	file, err = openCSV(dataFile.Name())
	assert.Nil(t, err)
